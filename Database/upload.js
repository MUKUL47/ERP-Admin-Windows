...
module.exports.updateMarkSheet = (req, res, err) => {
    ...
    upload(req, res, (err) => {
        new Promise((resolve, reject) => {
            ...
            }).then(msg => res.render("upload.ejs", {
                userPass: msg
            }))
            .catch(err => res.render("upload.ejs", {
                userPass: err
            }))
    })
}
    ...
    upload(req, res, (err) => {
        new Promise((resolve, reject) => {
                ...
                res.render("upload.ejs", {
                    userPass: "Notice file[s] uploaded"
                }))
            .catch(err =>
                res.render("upload.ejs", {
                    userPass: "Error uploading file : " + fileName
                })
            )
    })
    ...
    let retrieveFiles = () => {
        req.files.forEach((f) => {
            ...
            if (extension.length == 2 && extension[extension.length - 1].toLowerCase() === "csv") {
                ...
                return Promise.reject(`Invalid file name : ${f.originalname} (supports only "csv" ex : somefilename.csv) Uploaded remaining files :`)
            }
        })
        return Promise.resolve("Userdetails updated : ")
    }
    retrieveFiles().then(msg =>
        res.render("upload.ejs", {
            userPass: msg + filesUploaded.join(', ')
        })
    ).catch(err => res.render("upload.ejs", {
        userPass: err + filesUploaded.join(', ')
    }))
